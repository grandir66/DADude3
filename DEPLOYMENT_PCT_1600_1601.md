...
ssh root@192.168.40.1 "pct status 1600"

# Se non esiste, crealo (esempio configurazione)
# ssh root@192.168.40.1 "pct create 1600 ..."
```
...
ssh root@192.168.40.1 "pct set 1600 --net0 name=eth0,bridge=vmbr0,ip=192.168.4.100/24,gw=192.168.4.1"

# Avvia container
ssh root@192.168.40.1 "pct start 1600"

# Verifica connettività
ssh root@192.168.40.1 "pct exec 1600 -- ping -c 2 8.8.8.8"
```
...
```bash
ssh root@192.168.40.1 "pct exec 1600 -- bash -c '
...
# Trasferisci sul PCT
scp dadude-native.tar.gz root@192.168.40.1:/tmp/
...
scp root@192.168.40.1:/tmp/dadude_backup.dump /tmp/
scp /tmp/dadude_backup.dump root@192.168.40.1:/tmp/
```
...
# Verifica log
ssh root@192.168.40.1 "pct exec 1600 -- journalctl -u dadude-server -f"
...
ssh root@192.168.40.1 "pct set 1601 --net0 name=eth0,bridge=vmbr0,ip=192.168.4.101/24,gw=192.168.4.1"
ssh root@192.168.40.1 "pct start 1601"
```
...
scp dadude-native.tar.gz root@192.168.40.1:/tmp/
ssh root@192.168.40.1 "pct exec 1601 -- bash -c '
cd /opt
tar xzf /tmp/dadude-native.tar.gz -C /opt
...
# Verifica log
ssh root@192.168.40.1 "pct exec 1601 -- journalctl -u dadude-agent -f"

# Verifica connessione sul server
ssh root@192.168.40.1 "pct exec 1600 -- journalctl -u dadude-server | grep -i connected"
```
...
```bash
# Ferma container
ssh root@192.168.40.1 "pct stop 1600"

# Cambia IP a 192.168.4.45
ssh root@192.168.40.1 "pct set 1600 --net0 name=eth0,bridge=vmbr0,ip=192.168.4.45/24,gw=192.168.4.1"

# Riavvia
ssh root@192.168.40.1 "pct start 1600"

# Verifica IP
ssh root@192.168.40.1 "pct exec 1600 -- ip addr show eth0"
```
...
# Verifica log server
ssh root@192.168.40.1 "pct exec 1600 -- journalctl -u dadude-server | tail -20"
```
...
ssh root@192.168.40.1 "pct stop 600"
# ssh root@192.168.40.1 "pct destroy 600"  # ATTENZIONE: Solo se sicuro!
```
